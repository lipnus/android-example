...
public class S2Service {
    ...
    private List<Event> adidEvents;

    private int mBatchSize = 10; // default batch size = 10
    ...
    public void addEvent(final Event event) {
        addEventInternal(event, false, mAddEventCallback);
    }
    ...
    public void addEvent(final Event event, final ResponseCallback<Integer> callback) {
        addEventInternal(event, false, callback);
    ...
    public void addAdidEvent(final Event event) {
        addEventInternal(event, true, mAddEventCallback);
    }
    ...
    public void addAdidEvent(final Event event, final ResponseCallback<Integer> callback) {
        addEventInternal(event, true, callback);
    }

    @SuppressWarnings("WeakerAccess")
    void addEventInternal(final Event event, final boolean isAdidEvent, final ResponseCallback<Integer> callback) {
        int eventsSize;
        try {
            if (isAdidEvent) {
                adidEvents.add(event);
                eventsSize = adidEvents.size();
                if (adidEvents.size() >= mBatchSize) {
                    publishEventsInternal(adidEvents, mPublishCallback);
                }
            } else {
                ...
                if (events.size() >= mBatchSize) {
                    publishEventsInternal(events, mPublishCallback);
                }
            }
            if (callback != null) {
    ...
    public void publishEvents(final ResponseCallback<EventsLogResponse> callback) {
        publishEventsInternal(events, callback);
        publishEventsInternal(adidEvents, callback);
    }

    @SuppressWarnings("WeakerAccess")
    void publishEventsInternal(final List<Event> events, final ResponseCallback<EventsLogResponse> callback) {
            ...
            KakaoTaskQueue.getInstance().addTask(new KakaoResultTask<EventsLogResponse>(callback) {
                ...
                public EventsLogResponse call() throws Exception {
                    ...
                    if (events == adidEvents) {
                        ...
                        rootEvent = new Event(idInfo.getId(), !idInfo.isLimitAdTrackingEnabled());
