		...
		for (int i = 0; i < countColumn; i++) {
			...
			case MType.BYTE:
				...
				if (checkNull(outobj)) {
					...
					result[i] = TypeProcessor.toBytes(((Byte) outobj)
							.byteValue());
				}
			...
			case MType.SHORT:
				...
				if (checkNull(outobj)) {
					...
					result[i] = TypeProcessor
							.toBytes(((Short) outobj).shortValue());
				}
			...
			case MType.INTEGER:
				...
				if (checkNull(outobj)) {
					...
					result[i] = TypeProcessor
							.toBytes(((Integer) outobj).intValue());
				}
			...
			case MType.LONG:
				...
				if (checkNull(outobj)) {
					...
					result[i] = TypeProcessor.toBytes(((Long) outobj)
							.longValue());
				}
			...
			case MType.DOUBLE:
				...
				if (checkNull(outobj)) {
					...
					result[i] = TypeProcessor
							.toBytes(((Double) outobj).doubleValue());
				}
